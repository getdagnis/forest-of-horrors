...
| 2026-08-16 / radar-tower diagnostic map | `ServerScriptService.RadarTowerSystem`; `StarterPlayerScripts.RadarTowerHud` | Pending fresh Play test | Sitting in a Seat/VehicleSeat below a `RadarTower=true` tower opens a near-full-screen map with every living Drooling Zombie, the local player, and optional `radar_detect=true` landmarks. Clicking/tapping the console also opens it: the tower's existing `RadarSystem > RadarScreen > Drag` hierarchy is recognised automatically. `RadarMapMin`/`RadarMapMax` Workspace Vector3 attributes tune map bounds. |
...
| 2026-08-16 / mounted tower score bridge | Add `ReplicatedStorage.TurretCombatConfig`; add `ServerScriptService.TurretCombatSystem`; replace `StarterPlayerScripts.GunClientController`; add one `TurretShot:FireServer(...)` line to the legacy tower `control` template | Pending fresh Play test | Each actual legacy yellow bullet now reports through its own proven `control:fire()` path to a server-validated 8-damage / 0.10s Spherecast from the tower's `gunpart`/`gunpart2` barrels. Hits use `MonsterManager` for normal score/kill attribution, damage labels, monster flash, and permanent bomb kills. `bullet` remains visual-only. |
| 2026-08-16 / WMT legacy NPC error flood | Replace `ServerScriptService.GuardianMonsterSystem` | Pending fresh Play test | When the central guardian system registers a WMT, it now disables the marketplace `NPC` Script alongside its old touch-damage scripts. That old Script repeatedly indexed `BombMonster.HumanoidRootPart`, although BombMonster correctly uses a body/PrimaryPart instead, causing the high-frequency Output flood and needless heartbeat work. |
| 2026-08-16 / mounted tower controller modernisation | Replace the enabled `Script` below each mounted-gun `Seat`, and its disabled `control` LocalScript child | Pending fresh Play test | The legacy controller no longer waits forever for `TurretShot`, creates client-only damaging rockets, or listens to every Seat property change. The modern seat owner clones one controller for its current occupant; the client controller provides local rotation, sound/smoke/recoil, and harmless tracer presentation, while `TurretCombatSystem` remains the server-only damage/score owner. Keep `control` and `bullet` disabled as templates; `bullet` is no longer cloned. |
| 2026-08-16 / startup Output triage | Replace `ServerScriptService.RadarTowerSystem`, `ServerScriptService.DarkForestMusicSystem`, and `StarterPlayerScripts.McDonaldsJukeboxClient` | Pending fresh Play test | The central radar system disables the obsolete HologramGui `RadarScript` inside marked towers, ending its repeated `updateGUI` nil errors without altering the physical console. Forest music reports one concise missing-track warning instead of ten. The optional McDonald's jukebox client exits cleanly if the old emitter is absent rather than producing an infinite-yield warning. |
| 2026-08-17 / turret client bridge diagnostics | Replace `StarterPlayerScripts.TurretCombatClient` LocalScript | Pending fresh Play test | The client independently detects a player seated below a `Turret=true` model and fires `TurretShot` at the normal 0.10s cadence. It records `TurretClientStatus` / `TurretClientLastShotAt` on the outer turret and emits concise `[TurretCombatClient]` state messages, so a missing controller-to-server handoff is visible immediately without changing imported turret visuals. |
| 2026-08-17 / turret actual-seat validation | Replace `ServerScriptService.TurretCombatSystem` | Pending fresh Play test | Marketplace turret models can contain more than one Seat. The server now searches all descendant Seats/VehicleSeats for the one occupied by the firing player instead of rejecting the shot because Roblox returned an unrelated first Seat. |
| 2026-08-17 / MonsterGun contour callback crash | Replace `ServerScriptService.MonsterGunDamageSystem` | Pending fresh Play test | Declares the equipped-tool helper before the contour RemoteEvent callback that invokes it. The previous declaration order made every contour update call nil at line 151, preventing the normal gun damage path from being reached. |
| 2026-08-17 / turret exact SeatPart validation + crosshair | Replace `ServerScriptService.TurretCombatSystem` and `StarterPlayerScripts.TurretCombatClient` | Pending fresh Play test | The server now validates `player.Character.Humanoid.SeatPart` directly and requires that exact Seat/VehicleSeat to be below the marked turret. This avoids unreliable decorative-seat `Occupant` values. The global turret client shows the original yellow crosshair only while seated at the turret, then restores the prior cursor. |
| 2026-08-17 / turret server ownership bridge | Replace the enabled `Script` below the turret's actual gunner Seat, its disabled `control` LocalScript child, and `ServerScriptService.TurretCombatSystem` | Pending fresh Play test | The mounted Seat script records its current occupant's outer turret in a server-owned `Player.ActiveMountedTurret` ObjectValue. Turret combat now validates that exact record, instead of attempting to rediscover a Seat in the imported multi-Seat hierarchy. The controller also uses the occupying Humanoid's real SeatPart rather than the first decorative descendant, restoring its yellow turret cursor and correct cleanup. |
| 2026-08-17 / turret imported-seat fallback | Replace `ReplicatedStorage.TurretCombatConfig`, `ServerScriptService.TurretCombatSystem`, and `StarterPlayerScripts.TurretCombatClient` | Pending fresh Play test | Some marketplace seats still fail both ownership paths despite producing the old controller's shots. A fixed turret now accepts only a player who is genuinely sitting within 24 studs of that tower. Server fire-rate, barrel-origin Spherecast, range, and monster validation still apply. The client uses the same bounded rule for the yellow crosshair. |
| 2026-08-17 / explicit MachineGunSeat contract | Rename the actual gunner Seat to `MachineGunSeat`; replace `ReplicatedStorage.TurretCombatConfig`, `ServerScriptService.TurretCombatSystem`, and the Seat's disabled `control` LocalScript | Pending fresh Play test | The tower now names and selects its sole intended gameplay Seat directly. Combat and controller cleanup no longer choose an arbitrary decorative Seat from the marketplace model. |
| 2026-08-17 / turret legacy controller compatibility fallback | Replace `ReplicatedStorage.TurretCombatConfig`, `ServerScriptService.TurretCombatSystem`, and `StarterPlayerScripts.TurretCombatClient` | Pending fresh Play test | The legacy controller demonstrably sends shots, while its custom model fails to expose both `Humanoid.SeatPart` and `Humanoid.Sit` on the server. As a final bounded fallback, firing is allowed only while the player's character is within 100 studs of the fixed marked turret; hit validation, fire rate, and damage stay server-owned. The cursor appears within 30 studs without depending on the broken seat state. |
| 2026-08-17 / turret bad-pivot repair | Replace `ServerScriptService.TurretCombatSystem` and `StarterPlayerScripts.TurretCombatClient` | Pending fresh Play test | Runtime evidence proved the outer `Turret` Model pivot is far from its physical tower, falsely rejecting even a 100-stud operator radius. All player-distance and cursor checks now measure from the named `MachineGunSeat` Part instead of `Model:GetPivot()`. |
| 2026-08-17 / turret custom-controller authority boundary | Replace `ServerScriptService.TurretCombatSystem` | Pending fresh Play test | Play evidence proves the legacy emplacement provides no server-replicated relationship between its firing controller and Player character: it is neither seated nor physically near `MachineGunSeat`. The system therefore accepts only the marked Workspace turret identity and keeps actual combat fully server-side: rate limit, real barrel Spherecast, registered-monster lookup, damage, score, and permanent bomb kills. |
| 2026-08-17 / explicit radar tower pilot | Replace `ServerScriptService.RadarTowerSystem` and `StarterPlayerScripts.RadarTowerHud`; mark one outer tower, its real radar Seat, and `RadarScreen.Drag` | Pending fresh Play test | The radar no longer guesses from imported `RadarScreen`/Seat names. `RadarTower=true` marks the outer pilot, `RadarTowerSeat=true` marks only its real Seat, and `RadarTowerScreen=true` marks its physical `Drag` Part. The HUD renders every living `MonsterManager` monster, the local player, and other living players as grey markers. `RadarYacht` is local proximity ambience around the marked screen and `RadarLoop` is local while the HUD is open. Marked towers disable their legacy `RadarScript`; retain `[RadarTower]` startup/disable logs while validating the first model. |
